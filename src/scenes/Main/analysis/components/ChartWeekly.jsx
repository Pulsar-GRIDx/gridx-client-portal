 ...
 * @param {Array.<Object>} params.chartSeries - The data series for the chart.
 * @returns {Object} The chart options.
 */
const useChartOptions = ({ xaxisTitle, yaxisTitle, chartSeries }) => {
  const theme = useTheme();
  const colors = tokens(theme.palette.mode);

  const maxValue = Math.max(
    ...chartSeries.map((series) => Math.max(...series.data)),
  );

  const xaxisMax = maxValue * 1.25;
...
/**
 * A React component that renders a weekly chart using ReactApexChart.
 *
 * @component
 * @memberof AnalysisDash.Analysis_components
 * @param {Object} props - The props for the component.
 * @param {Array.<Object>} props.chartSeries - The data series for the chart.
 * @param {string} props.xaxisTitle - The title for the x-axis.
 * @param {string} props.yaxisTitle - The title for the y-axis.
 * @returns {JSX.Element} The rendered component.
 */
const ChartWeekly = ({ chartSeries, xaxisTitle, yaxisTitle }) => {
  const chartOptions = useChartOptions({ xaxisTitle, yaxisTitle, chartSeries });
  const theme = useTheme();
  const colors = tokens(theme.palette.mode);
